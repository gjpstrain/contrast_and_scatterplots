...
labels_e2 <- c(A = "Non-Linear Decay", B = "Linear Decay", C = "Inverted Decay", D = "No Decay")
```
...
the messages that visualizations are trying to display. It is for this reason that
...
much easier to discern for the user. Reducing the contrast of all points in such
a way increases their levels of transparency, meaning the colour intensity of 
regions with more overlapping points sum. Our stimuli had only 128 small points, meaning
the majority of points were clearly visible at all times. For this reason, and due to
...
this would necessitate hiding data, thus changing the information presented to the viewer.
...
unable to obtain precise monitor resolution or physical screen size measurements for 
participants, meaning we are unable to provide by-participant dot pitch information.
...
Scatterplots were generated according to the standard normal distribution, having
a standard deviation of 1 in each direction.
...
highest contrast plots (alpha = 1), an effect size of d = `r printnum(effects_df$d[3])`
...
knitr::kable(contrasts_e1, booktabs = TRUE, digits = c(0,2,3), caption = "\\label{contrasts-table-e1}This table shows the contrasts between different levels of the contrast factor in E1.")
```
...
invariance in accuracy and precision with regards to contrast. Our results, in comparison 
to these previously reported findings, do find an effect, which we argue is due to 
differences in methodology and experimental power. Rensink used bisection and JND tasks to fit equations for accuracy and precision. Our methodology simply asked participants to estimate correlation, producing comparative judgements more
suited to informing scatterplot design than investigating a mathematical relationship between
correlation and perceived correlation allowed by other methodologies. The
...
high and full contrast conditions in experiment 1, we hypothesise that this is due
...
contrasts_e2 <- contrasts_extract(e2_model) %>%
  mutate(p.value = scales::pvalue(p.value)) %>%
  mutate(Contrast = recode(Contrast,
                           "D - C" = "Full contrast (D) : Non-linear inverted decay (C)",
                           "D - B" = "Full contrast (D) : Linear decay (B)",
                           "D - A" = "Full contrast (D) : Non-linear contrast decay (A)",
                           "C - B" = "Non-linear inverted decay (C) : Linear decay (B)",
                           "C - A" = "Non-linear inverted decay (C) : Non-linear contrast decay (A)",
                           "B - A" = "Linear decay (B) : Non-linear contrast decay (A)"))
...
```{r e2-sd-sd-plot, warning=FALSE, echo=FALSE, message=FALSE, fig.align='left', out.width = "50%", fig.cap="\\label{e2-sd-sd-plot}The mean of standard deviations of participants in experiment 2, grouped by contrast condition.", description = "Dot plot showing means of standard deviations of participants in experiment 2."}
...
Figure \ref{e2-sd-sd-plot} shows how participant standard deviations vary
...
and \ref{e2-changes-plot} plot the objective *r* value against the mean correlation estimates
against Pearson's *r* for experiments 1 and 2 respectively. Standard deviations of 
...
with the concept. To test if the patterns we observed in correlation perception were as a result of this training,
we built a model including the half of the session (first or second) as a predictor.
Comparing this to the original models not including session half as a predictor revealed
...
decay function) had a greater influence on correlation estimation than any effect of
training.
...
it still allows us to inform design research.
...
However, given previous work [@sher_2017] which found evidence for people overestimating
