...
plot_sd_sd_function <- function(d, title_text){
  d %>% 
  summarise(ppt_sd = sd(error)) %>% 
  drop_na() %>% 
  group_by(contrast) %>% 
  summarise(mean_sd = mean(ppt_sd), sd_sd = sd(ppt_sd)) %>% 
  ggplot(aes(x = contrast, y = mean_sd)) +
  geom_point(size = 2) + 
  geom_errorbar(aes(ymin = mean_sd - sd_sd, ymax = mean_sd + sd_sd), width = 0, size = 1) + 
  theme_ggdist() +
  ylim(0, .3) +
  lans(x = "Contrast Condition",
...
```{r e1-sd-sd-plot, warning=FALSE, echo=FALSE, message=FALSE, fig.align='left', out.width = "50%", fig.cap="\\label{e1-sd-sd-plot}The mean of standard deviations of participants in experiment 1, grouped by contrast condition.", description = "Dot plot showing means of standard deviations of participants in experiment 1."}
...
correlation estimates are shown in figure \ref{dot-plot-e2}.The **EMAtools**
package (version 0.1.4 [@ematools]) was used to calculate effect sizes in Cohen's d. For
...
present a number of difficulties. As can be seen in figures \ref{dot-plot-e1}
and \ref{dot-plot-e2}, and as more specifically illustrated in Figure \ref{e2-dist-plots}, 
