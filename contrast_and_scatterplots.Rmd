...
reduced contrast, for example speed perception in @champion_2017. With this in mind,
...
lower contrast would have a more variable spread of correlation estimates than plots with 
...
and experimental power. Rensink used bisection and JND tasks to fit equations for accuracy and precision. Our methodology simply asked participants to estimate correlation, producing comparative judgements more
...
more accurate and more precise when the *r* value was higher. Figure \ref{e2-dist-plots} 
plots the objective *r* value against the mean correlation estimate
...
of precision and accuracy for *r* values further from 0 or 1.

```{r e2-dist-plots, warning=FALSE, echo=FALSE, message=FALSE, fig.show='hold', out.width="50%", fig.cap="\\label{e2-dist-plots}Plotting objective \\textit{r} values against mean error in correlation estimates (on the left) and standard deviations of correlation estimates (on the right) shows that accuracy increases with \\textit{r}.", description = "Plot showing objective r value on the horizontal axis, and mean error by r value on the vertical axis (left), and standard deviation of mean errors on the vertical axis (on the right). Error is lowest when r is lowest and highest, and follows a curve reaching a zenith at around r = 0.6."}
...
scatterplots (e.g @cleveland_1982; @meyer_1997; @yang_2019; @rensink_2017). 
...
should utilise the technique we have described here to give visualization viewers
...
and \ref{sum-stats-e2}, and as more specifically illustrated in Figure \ref{e2-dist-plots}, 
...
Although we have presented a novel visualization technique that has resulted in
